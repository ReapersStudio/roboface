...
export const DEFAULT_REACTION_RECORD = DEFAULT_REACTIONS.reduce((record, reaction) => {
...
export const DEFAULT_SETTINGS = {
  ...
  selection: {
    // unified ordered list of reactions + widgets shown in the "In use" box.
    // { t: "r", id } = reaction,  { t: "w", key } = widget
  ...
  firebase: {
    ...
    projectId: "",
    databaseURL: "",
    status: "Configured with Vite environment variables",
  },
  overlay: {
    enabled: false,
    showDate: false,
...
export const reactionToDeviceFields = (reaction) => ({
...
export const blankReaction = (nextCode, order) => ({
  id: `custom-${Date.now()}`,
  code: nextCode,
...
const normalizeReaction = (reaction, fallback = DEFAULT_REACTIONS[0]) => {
...
const normalizeReactionRecord = (incoming = {}) => {
  const source = incoming && Object.keys(incoming).length ? incoming : DEFAULT_REACTION_RECORD;

  return Object.values(source).reduce((record, reaction) => {
...
export const normalizeState = (incoming = {}) => {
  ...
  const currentFromCode = orderedReactions.find(
    (reaction) => reaction.code === incoming.control?.currentReactionCode,
  );
